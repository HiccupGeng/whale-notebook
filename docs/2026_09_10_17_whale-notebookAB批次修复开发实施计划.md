...
> 日期：2026-09-10 17 时 ｜ 作者：DSH 会话（待署名） ｜ 版本：v1.1（S1–S4 已执行并通过验证；S5–S8 待宿主重启窗口，见 §9）
...
**尚未执行（S5–S8）**：版本口径收口与部署/发布是**一个原子步骤**——若只把源与文档改到 0.7.3 而不部署，就会造出反向漂移（源写 0.7.3、`/whale/live` 仍自报 0.7.2），与本次要修的问题同类。故 S5 与 S6/S7/S8 一起做，起点是**用户的宿主重启窗口**。
