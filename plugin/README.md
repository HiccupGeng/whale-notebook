...
- 验证（2026-09-12 实测全绿，v0.7.7）：`node src/ui/server.selftest.cjs`(63) + `src/core/privacy.selftest.cjs`(23) + `src/core/summarize.selftest.cjs`(10) + `src/core/similarity.selftest.cjs`(20) + `src/store/repo.selftest.cjs`(34，v0.7.7 +6 维护标记) + `src/collector/engine.selftest.cjs`(35，v0.7.7 +9 异步/窗口化/维护窗口) + `src/collector/engine.dedup.selftest.cjs`(24) + `src/collector/e2e.selftest.cjs`(67，v0.7.7 +1) + `src/collector/live.selftest.cjs`(48，v0.7.7 +3 维护让路) + `lifecycle/selftest.cjs`(120，v0.7.7 +16 漂移分级/adopt) = **10 套件 444 断言** + `node scripts/bundle-smoke.cjs`（bundle 桩）+ `scripts/redact.test.cjs`(22 断言) + `scripts/links-doctor.selftest.cjs`(43 断言)，全绿（13 个测试文件 PASS 累计 537）。
