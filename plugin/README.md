# dsh-whale-notebook 插件包（v0.4.0：双半插件 · 决策箱双卡 = 待审箱 + 已解决墙）
...
├─ lib/index.js            # 插件入口(host half: /whale/inbox|detail|solved|entry|delete 注册, v0.4)
├─ lib/client.js           # 浏览器半边(决策箱悬浮面板 bundle; v0.3: 红✕/判定表/RISK 警示; v0.4: 双卡 待审+已解决, __ModuleLoader__ 零依赖)
...
```
   ...
   │  └─ cli.cjs           # CLI 分发(含 --render-rules、v0.4 --wall 打印已解决墙全文)
...
设计文档：项目 `docs/2026_09_09_18_whale-notebook决策箱面板设计.md`（v2.1 基础）+ `docs/2026_09_09_22_whale-notebook决策箱v0.3实施计划.md` + `docs/2026_09_09_23_whale-notebook已解决墙与分类v0.4实施计划.md`。浏览器半边=悬浮侧边面板**双卡**：待审箱（⚡自动处理/💬详细讨论/✕删除）+ 已解决墙（全局区/项目区分组，行点击展开条目全文）；host 半边注册 `GET /whale/inbox`、`GET /whale/inbox/detail`、`GET /whale/solved`、`GET /whale/entry` 与 `POST /whale/inbox/delete`。
...
- 验证：`node src/ui/server.selftest.cjs`（host 逻辑 45 断言）、`node src/core/privacy|summarize.selftest.cjs`、`node src/collector/engine|e2e.selftest.cjs`（engine 10 + zstd 全链 11 断言）、`node scripts/bundle-smoke.cjs`（bundle 桩，含 v0.4 结构断言）——共 5 套件 86 断言 + bundle 桩，全绿（2026-09-09 v0.4.0）。
