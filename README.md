...
**当前版本 `0.7.3`** ｜ 全部自检 **320 断言全绿** ｜ 用户记忆数据**只在本机、永不入库**
...
node scripts/redact.test.cjs                          # 13 PASS · 打码回归
```

合计 **9 套自检 320 断言** + bundle 桩 + 13 打码断言 + 43 死链体检断言 + 28 讨论落点断言（最近全绿：2026-09-10）。所有自检都在临时沙盒里跑，**不触碰真实 `~/.dsh`**。
