...
**当前版本 `0.7.7`** ｜ 全部自检 **442 断言全绿**（13 个测试文件 PASS 累计 535） ｜ 用户记忆数据**只在本机、永不入库**
