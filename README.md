...
**当前版本 `0.7.4`** ｜ 全部自检 **363 断言全绿**（13 个测试文件 PASS 累计 434） ｜ 用户记忆数据**只在本机、永不入库**
...
合计 **10 套自检 363 断言** + bundle 桩 + 22 打码断言 + 43 死链体检断言 + 28 讨论落点断言 ＝ 13 个测试文件 PASS 累计 **434**（最近全绿：2026-09-11）。所有自检都在临时沙盒里跑，**不触碰真实 `~/.dsh`**。
